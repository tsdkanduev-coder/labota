...
export class VoiceCallWebhookServer {
  ...
  private buildRealtimeConversationContext(providerCallId: string): RealtimeConversationContext {
    ...
    // wording of the greeting is generated by the model based on
    // `instructions` (which already contain objective / context).
    const shouldSpeakFirst = !!(objective || initialMessage);

    return {
      instructions,
      initialPrompt: shouldSpeakFirst ? "start" : undefined,
      language,
